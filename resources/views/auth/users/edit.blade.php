                ...
                <x-form.section>
                    ...
                    <x-slot name="body">
                        <div class="sm:col-span-6 grid gap-x-8 gap-y-6 {{ user()->id == $user->id ? 'grid-rows-3' : 'grid-rows-2' }}">
                            ...
                            @if (user()->id == $user->id)
                            <x-form.group.checkbox name="change_password" :options="['1' => trans('auth.change_password')]" form-group-class="sm:col-span-6" checkbox-class="sm:col-span-6" @input="onChangePassword($event)" />
                        ...
                        <div class="sm:col-span-6 grid gap-x-8 gap-y-6 {{ user()->id == $user->id ? 'grid-rows-3' : 'grid-rows-2' }}">
                            ...
                            @else
                                <x-form.group.file name="picture" :value="$user->picture" label="{{ trans_choice('general.pictures', 1) }}" not-required form-group-class="sm:col-span-6 sm:row-span-2" />
                            @endif
