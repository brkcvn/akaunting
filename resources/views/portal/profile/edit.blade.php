        ...
        <x-form.container>
            <x-form id="portal" method="PATCH" :route="['portal.profile.update', $user->id]" :model="$user">
                <x-form.section>
                    ...
                    <x-slot name="body">
                        <div class="sm:col-span-6 grid gap-x-8 gap-y-6 {{ user()->id == $user->id ? 'grid-rows-4' : 'grid-rows-3' }}">
                            ...
                            <x-form.group.checkbox name="change_password" :options="['1' => trans('auth.change_password')]" form-group-class="sm:col-span-6 {{ user()->id == $user->id ? '' : 'hidden' }}" input-group-class="pt-8" @input="onChangePassword($event)" />
                        ...
                        <div class="sm:col-span-6 grid gap-x-8 gap-y-6 {{ user()->id == $user->id ? 'grid-rows-4' : 'grid-rows-3' }}">
                            @if (setting('default.use_gravatar', '0') == '1')
                                <x-form.group.text name="fake_picture" label="{{ trans_choice('general.pictures', 1) }}" disabled placeholder="{{ trans('settings.default.use_gravatar') }}" form-group-class="sm:col-span-6 sm:row-span-3" />
                            @else
                                <x-form.group.file name="picture" label="{{ trans_choice('general.pictures', 1) }}" :value="$user->picture" not-required form-group-class="sm:col-span-6 sm:row-span-3" />
                            @endif
